...
public class Portal : MonoBehaviour
{
    ...
    public Transform destinationForth;
        ...
        //if (gm.isTeleporting == false)
        {
            if (other.CompareTag("Player") && other.TryGetComponent<PlayerController>(out var player))
            {
                if (gm.currentRoom != null)
                {
                    ...
                    {
                        DestinationForth();
                        player.Teleport(destinationForth.position);
    ...
    Transform DestinationForth()
    {
        switch (gm.currentRoom.tag)
        {
            case "Entrance":
                destinationForth = d.DestinationsKitchen[Random.Range(0, d.DestinationsKitchen.Length)];
                Debug.Log("case entrance, random kitchen");
                break;
            case "Kitchen":
                if (gm.isRoomCorrect[1]) //!!!!!!!!!!!!! in progress
                {
                    destinationForth = d.DestinationsLiving[Random.Range(0, d.DestinationsLiving.Length)];
                }
                else
                {
                    destinationForth = d.DestinationsLiving[Random.Range(1, d.DestinationsLiving.Length)];
                }
                
                Debug.Log("case kitchen, random living");
                break;
            case "Living":
                if(gm.isRoomCorrect[2])
                {
                    destinationForth = d.DestinationsBathroom[Random.Range(0, d.DestinationsBathroom.Length)];
                }
                else
                {
                    destinationForth = d.DestinationsBathroom[Random.Range(1, d.DestinationsBathroom.Length)];
                }
            ...
            case "Bathroom":
                ...
                {
                    destinationForth = d.DestinationsBedroom[Random.Range(1, d.DestinationsBedroom.Length)];
                }
                Debug.Log("case bathroom, random bedroom");
                break;
            case "Bedroom":
                destinationForth = d.EntranceDestination;
    ...
    Transform DestinationBack()
    {
        switch (gm.currentRoom.tag)
        {
            case "Kitchen":
                ...
                else { destinationBack = d.DefaultDestination; }
